...
public class Inventory { //hold all the items with their data
    ...
    public class InventorySlot { //script for each slot within the inventory
        ...
        public ItemObject itemobject { //makes it a public accesible with only a readonly
            ...
            {
                if (item.Id >= 0) {
                    return parentInventroy.inventory.database.Items[item.Id];
                }
        ...
        public void DropItem () { //drops the item
            if (itemamount > 1) { //if it has more than 1 in the amount it decreases amount by 1 and drops the item
                ...
                ItemObject droppeditem = parentInventroy.inventory.database.Items[item.Id];
                Pickup_Interact.instance.Drop (droppeditem);
